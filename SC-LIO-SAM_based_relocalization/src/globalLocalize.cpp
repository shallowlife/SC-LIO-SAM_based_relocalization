...
public:
	...
	//std::cout << "come in3" << std::endl;
        ...
        {
            if (is_regular_file(entry)){count++; }
        }
        std::cout << "Total number of files: " << count << std::endl;
        ...
        const float kSCFilterSize = 0.5; // zxl
        downSizeFilterSC.setLeafSize(kSCFilterSize, kSCFilterSize, kSCFilterSize); //zxl
        //zxl testing loadSCD
        // Eigen::IOFormat OctaveFmt(StreamPrecision, 0, ", ", ";\n", "", "", "[", "]");
        // std::cout << scManager.polarcontexts_[0].format(OctaveFmt) << std::endl;
        // std::cout << scManager.polarcontexts_.size() << std::endl;

        //zxl load_poses
        loadPosesDirectory = std::getenv("HOME") + savePCDDirectory +  "optimized_poses.txt"; 
        loadPoses(loadPosesDirectory, matrix_poses);
        //std::cout << load_poses.size() << std::endl;
    ...
    void laserCloudInfoHandler(const lio_sam::cloud_infoConstPtr& msgIn)
    {
        ...
        pcl::fromROSMsg(msgIn->cloud_deskewed,  *laserCloudRaw); // zxl
        laserCloudRawTime = cloudInfo.header.stamp.toSec(); // zxl
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    //zxl(have changed,different from sc-liosam)
    void loopFindNearKeyframesWithRespectTo(pcl::PointCloud<PointType>::Ptr& nearKeyframes, const int& key, const int& searchNum, const int _wrt_key)
    {
        ...
        nearKeyframes->clear();
        int cloudSize =  scManager.polarcontexts_.size();//静态点云帧和SCD帧一一对应，数量相同
    ...
    {
        // find keys
        auto detectResult = scManager.detectLoopClosureID(); // first: nn index, second: yaw diff 这个函数引起的崩溃，要排查
        ...
        int base_key = 0;
        //pcl::copyPointCloud(*laserCloudRawDS,  *cureKeyframeCloud);
        //将当前帧利用匹配上帧的位姿，变换到一致的坐标系
        ...
        *cureKeyframeCloud += *transformPointCloud(laserCloudRawDS, &pointTypePose_temp);
        ...
        /***test visualization*****/
        // pcl::visualization::CloudViewer viewerPre("NearKeyframes");
        // viewerPre.showCloud(prevKeyframeCloud);
        // while(!viewerPre.wasStopped()){};
        // pcl::visualization::CloudViewer viewerCur("cureKeyframeCloud");
        // viewerCur.showCloud(cureKeyframeCloud);
        // while(!viewerCur.wasStopped()){};
        /***test visualization****/
        ...
        Eigen::Affine3f loopKeyPreTransformInTheWorld =  pclPointToAffine3f(pointTypePose_temp);//zxl
        ...
        std::cout << "the pose loop is: x" << x << " y" << y
                  << " z" << z <<std::endl;
	...
	//cloudScanForInitialize->clear();
        ...
        // pcl::PointCloud<PointType>::Ptr cureKeyframeCloud(new pcl::PointCloud<PointType>());
        // PointTypePose pointTypePose_temp;
        // pointTypePose_temp.x = transformInTheWorld[3];
        // pointTypePose_temp.y = transformInTheWorld[4];
        // pointTypePose_temp.z = transformInTheWorld[5];
        // pointTypePose_temp.roll = transformInTheWorld[0];
        // pointTypePose_temp.pitch =transformInTheWorld[1];
        // pointTypePose_temp.yaw = transformInTheWorld[2];
        // *cureKeyframeCloud += *transformPointCloud(laserCloudIn, &pointTypePose_temp);//zxl
        /****test*****/

        /***test visualization*****/
        // pcl::visualization::CloudViewer viewerPre("cloudGlobalMapDS");
        // viewerPre.showCloud(cloudGlobalMapDS);
        // while(!viewerPre.wasStopped()){};
        // pcl::visualization::CloudViewer viewerCur("cureKeyframeCloud");
        // viewerCur.showCloud(cureKeyframeCloud);
        // while(!viewerCur.wasStopped()){};
	...
	publishCloud(&pubMapWorld, cloudGlobalMapDS, timeLaserInfoStamp, "map");
        ...
        if (icp.hasConverged() == false || icp.getFitnessScore() > historyKeyframeFitnessScore)
        {
            ...
            std::cout << "Initializing Fail" <<icp.hasConverged()<< std::endl;
            return;
        } else{
